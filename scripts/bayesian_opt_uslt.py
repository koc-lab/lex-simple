...
from utils.evaluation import cross_validate_evaluator
...
"""
legal_base_tokenizer = BertTokenizer.from_pretrained("nlpaueb/legal-bert-base-uncased")
...
#df_subtlex = pd.read_excel(r"C:/Users/mert.cemri-s/Desktop/mert.cemri/legal_text_simplification/SUBTLEX_frequency.xlsx")
...
#allvectorsfile = open("AllVectors.txt","r")
#vectorlines = allvectorsfile.readlines()
#law2vec = {}
"""
for line in vectorlines:
    scalars = line.split()
    word_key = str(scalars[0])
    word_embedding = np.array(scalars[1:])
    law2vec[word_key] = word_embedding
"""   
...
#=======
                                        """
>>>>>>> 2df61a250326a7664d25442730ef2888ed6303b8
                                        try:
                                            key_vector = law2vec[lemmatizer.lemmatize(key)]
                                            candidate_vector = law2vec[lemmatizer.lemmatize(candidate)]
                                            cos_sim_law2vec = float(dot(key_vector, candidate_vector)/(norm(key_vector)*norm(candidate_vector)))
                                            score += cos_sim_law2vec * weight_cos
                                            #law2vec_successful += 1
                                        except:
                                            #print("Warning: law2vec score could not be evaluated for the target: **"+key+"** and candidate: **"+sugg[0]+"**")
                                            #law2vec_failed += 1
                                            pass
<<<<<<< HEAD
...
>>>>>>> 2df61a250326a7664d25442730ef2888ed6303b8
                                        try:
                                            key_vector = law2vec[lemmatizer.lemmatize(eff_key)]
                                            candidate_vector = law2vec[lemmatizer.lemmatize(candidate)]
                                            cos_sim_law2vec = float(dot(key_vector, candidate_vector)/(norm(key_vector)*norm(candidate_vector)))
                                            score += cos_sim_law2vec * weight_cos
                                            law2vec_successful += 1
...
def black_box_function(bert_weight, lm_weight, cos_weight, freq_weight, len_weight):
    ...
    for line in simple_lines:
      ...
      if line != simple_lines[-1]:
        simple_file.write("\n")
    
    simple_file.close()
    """
    simplify_case(target_path,simple_path, legal_base_tokenizer, legal_base_model, complex_words, bert_weight, cos_weight, lm_weight ,freq_weight,len_weight)
    tuples, scores = cross_validate_evaluator(simple_path)
      
    these_params = [bert_weight, lm_weight, cos_weight, freq_weight, len_weight]
    f1 = scores[3]
    print(f"{f1}: {these_params}")
...
if __name__ == '__main__':
    ...
    pbounds = {'bert_weight': (0, 3), 'lm_weight': (0, 6), 'cos_weight': (0, 3), 'freq_weight': (0, 2), 'len_weight': (0, 6)}
    ...
    record = open("bayesian_optimization_records.txt","a")
    
    for i, res in enumerate(optimizer.res):
     record.write("Iteration {}: \n\t{}".format(i, res))
     record.write("\n")
